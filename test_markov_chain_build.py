...
class TestBaseHelperMethods:
    ...
    class test_update_from_edge_base:
        ...
        def test_increment_prob_mat_cell(self):
            ...
            final_prob_matrix = np.array([[0, 0], [1, 0]])

            mc = MarkovChain(('a', 'b'))

            assert mc._prob_mat == initial_prob_matrix

            mc._increment_prob_mat_cell('a', 'b')

            assert mc._prob_mat == final_prob_matrix


    class test_get_probability_vector:

        def test_get_equally_likely_prob_vect(self):
            expected_vector = np.array([0.5, 0.5])
            mc = MarkovChain(('a', 'b'))
            assert mc._get_equally_likely_prob_vect() == expected_vector

        def test_get_single_result_prob_vector(self):
            expected_vector = np.array([1, 0])
            mc = MarkovChain(('a', 'b'))
            assert mc._get_single_result_prob_vector('a') == expected_vector
